...
localparam int SIZE_WIDTH = $clog2(STRB_WIDTH);  
  ...
  clocking drv_cb_slave @(posedge aclk);
    default input #1 output #0;
    input aresetn;
        input awid,awvalid, awaddr,  awlen, awsize, awburst, awlock, awcache,  awprot, awqos,  awregion, awuser;
        output awready;
    ...
    endclocking
    clocking mon_cb_slave @(negedge aclk);
  default input #1 output #0;
...
endclocking
  ...
  modport S_DRV(clocking drv_cb_slave);
  modport S_MON(clocking mon_cb_slave);
endinterface
